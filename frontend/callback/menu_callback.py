...
from frontend.templates import BANK_FRONT, HELP_FRONT, MAIN_PAGE_FRONT, PREDICT_FRONT
...
ARGUMENTS_FOR_UPDATE_BALANCE = [
    Output("counter_bank", "children"),
    Input("button_bank", "n_clicks"),
]
ARGUMENTS_FOR_BANK_BUTTON_MAIN_MENU = [
    Output("hidden_div_for_redirect_callback", "children"),
    Input("button_main", "n_clicks"),
]
...
ARGUMENTS_FOR_UPDATE_PREDICT = [
    Output("predict_h2", "children"),
    Output("upload-data", "children"),
    Input("upload-data", "filename"),
    Input("upload-data", "contents"),
]
ARGUMENTS_FOR_GET_PREDICT = [
    Output("file-list", "children"),
    Output("predict", "style"),
    Input("predict", "n_clicks"),
]
ARGUMENTS_FOR_CHANGE_COLOR_BUTTON = [
    Output("predict", "style", allow_duplicate=True),
    Input("predict", "n_clicks"),
]
...
def test(n_intervals):
    try:
        n_clicks = (
            MoneyDB.get_money_by_username(session=session, username="admin") // 100
        )
    except:
        return 0
    return n_clicks
...
def bank_button_main_page(n_clicks):
    if n_clicks is not None and n_clicks > 0:
        return dcc.Location(href=URL_FOR_BANK, id="someid_doesnt_matter")
    else:
...
def get_predict(n_clicks):
    ...
    if n_clicks >= 1:
        if (
            (MODEL == "lr" and MONEY < 100)
            or (MODEL == "dt" and MONEY < 200)
            or (MODEL == "ct" and MONEY < 300)
        ):
            return html.P(
        ...
        with open(csv_file_path, "rb") as file:
            ...
            response = requests.post(
                URL_FOR_PREDICT, files=files, params={"model_name": MODEL}
            )
            
        if MODEL == "lr":
            MoneyDB.add_money_by_username(
                session=session, username="admin", amount=-100
            )
        elif MODEL == "dt":
            MoneyDB.add_money_by_username(
                session=session, username="admin", amount=-200
            )
        else:
            MoneyDB.add_money_by_username(
                session=session, username="admin", amount=-300
            )
